...
namespace DuckDBGeoparquet.Services
{
    public static class GersSql
    {
        ...
        public const string LinkagePolicyLinkDontReplace = "link_dont_replace";
        public const string SourceGeometryPolicyAuthoritativeLocal = "authoritative_local";
        public const string LinkageToolName = "arcgis_pro_gersify";
            ...
            string sourceLayerName = null)
        {
            ...
            string escapedRelease = Escape(string.IsNullOrWhiteSpace(overtureRelease) ? "unknown" : overtureRelease);
            string escapedSourceLayer = Escape(string.IsNullOrWhiteSpace(sourceLayerName) ? datasetName : sourceLayerName);
            string escapedLinkagePolicy = Escape(LinkagePolicyLinkDontReplace);
            string escapedGeometryPolicy = Escape(SourceGeometryPolicyAuthoritativeLocal);
            string escapedLinkageTool = Escape(LinkageToolName);
            ...
            return $@"
                ...
                COPY (
                    SELECT
                        ...
                        '{escapedType}' AS type,
                        '{escapedLinkagePolicy}' AS linkage_policy,
                        '{escapedGeometryPolicy}' AS source_geometry_policy,
                        CAST(a.match_strategy AS VARCHAR) AS match_strategy,
                        CAST(a.gers_match_score AS VARCHAR) AS match_score,
                        '{escapedRelease}' AS overture_release,
                        '{escapedSourceLayer}' AS source_layer,
                        '{escapedLinkageTool}' AS linkage_tool
                    FROM {AcceptedTable} a
                    JOIN {UserInputTable} u
                      ON a.record_id = u.record_id
