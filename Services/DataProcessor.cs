...
namespace DuckDBGeoparquet.Services
{
    ...
    public class LayerCreationInfo
    {
        ...
        public bool ShouldApplyOvertureStyling { get; set; }
    }
    ...
    public class DataProcessor : IDisposable
    {
        ...
        /// <param name="shouldApplyOvertureStyling">Whether to apply Overture Maps styling.</param>
        public async Task CreateFeatureLayerAsync(string layerNameBase, IProgress<string> progress, string parentS3Theme, string actualS3Type, string dataOutputPathRoot, bool shouldApplyOvertureStyling)
        {
            System.Diagnostics.Debug.WriteLine($"CreateFeatureLayerAsync called for: {layerNameBase}, ParentS3Theme: {parentS3Theme}, ActualS3Type: {actualS3Type}, DataOutputPathRoot: {dataOutputPathRoot}, ApplyStyling: {shouldApplyOvertureStyling}");
            ...
            try
            {
                // Now pass this specific folder and the styling flag to ExportByGeometryType
                await ExportByGeometryType(layerNameBase, themeTypeSpecificFolder, progress, shouldApplyOvertureStyling);
        ...
        private async Task ExportByGeometryType(string layerNameBase, string themeTypeSpecificOutputFolder, IProgress<string> progress = null, bool shouldApplyOvertureStyling = true)
        {
            System.Diagnostics.Debug.WriteLine($"ExportByGeometryType for {layerNameBase}, OutputFolder: {themeTypeSpecificOutputFolder}, ApplyStyling: {shouldApplyOvertureStyling}");
            ...
            foreach (var geomType in geometryTypes)
            {
                ...
                try
                {
                    ...
                    var layerInfo = new LayerCreationInfo
                    {
                        ...
                        ShouldApplyOvertureStyling = shouldApplyOvertureStyling // Use the passed parameter
                    };
                        ...
                        for (int i = 0; i < layers.Count && i < layerNames.Count; i++)
                        {
                            if (layers[i] != null)
                            {
                                ...
                                if (layers[i] is FeatureLayer featureLayer)
                                {
                                    ...
                                    // Apply Overture Maps styling if layer info is available and styling is requested
                                    if (i < sortedLayers.Count)
                                    {
                                        var currentLayerInfo = sortedLayers[i]; // Use currentLayerInfo
                                        if (currentLayerInfo.ShouldApplyOvertureStyling)
                                        {
                                            try
                                            {
                                                OvertureStyleManager.ApplyOvertureMapsStyling(featureLayer, 
                                                    currentLayerInfo.ParentTheme, 
                                                    currentLayerInfo.ActualType).Wait();
                                            }
                                            catch (Exception ex)
                                            {
                                                System.Diagnostics.Debug.WriteLine($"Warning: Failed to apply Overture styling to layer {featureLayer.Name}: {ex.Message}");
                                            }
                                        }
                                    }
                                }
        ...
        private async Task FallbackToIndividualLayerCreation(List<LayerCreationInfo> layers, IProgress<string> progress)
        {
            foreach (var layerInfo in layers)
            {
                try
                {
                    await AddLayerToMapAsync(layerInfo.FilePath, layerInfo.LayerName, progress, layerInfo);
                }
                catch (Exception ex)
                {
        ...
        private static async Task AddLayerToMapAsync(string parquetFilePath, string layerName, IProgress<string> progress = null, LayerCreationInfo layerInfo = null)
        {
            ...
            await QueuedTask.Run(() =>
            {
                ...
                try
                {
                    ...
                    if (newLayer is FeatureLayer featureLayer) // Apply to all FeatureLayers
                    {
                        ...
                        // Apply Overture Maps styling if layer info is available and styling is requested
                        if (layerInfo != null && layerInfo.ShouldApplyOvertureStyling)
                        {
                            try
                            {
                                OvertureStyleManager.ApplyOvertureMapsStyling(featureLayer, 
